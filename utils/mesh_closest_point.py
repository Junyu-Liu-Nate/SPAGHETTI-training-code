...
def vs_cp(vs: T, target_mesh: T_Mesh) -> TS:
    ...
    cp, distances, inds = mesh_trimesh.nearest.on_surface(vs.detach().cpu().numpy())
    cp, distances = torch.from_numpy(cp).to(device, dtype=vs.dtype), torch.from_numpy(distances).to(device)
    inds = torch.from_numpy(inds).to(device, dtype=torch.int64)
    return cp, distances, inds
