...
class GmmMeshStage:
    ...
    def __init__(self, opt: options.Options, shape_id: str, render: ui_utils.CanvasRender, render_number: int,
        ...
        if shape_id != '-1':
            mesh_dir = 'occ'
            gmms_dir = 'gmms'
            mesh_source = f'{opt.cp_folder}/{mesh_dir}/{shape_id}.obj'
            self.base_mesh = files_utils.load_mesh(mesh_source)
            self.raw_gmm = files_utils.load_gmm(f'{opt.cp_folder}/{gmms_dir}/{shape_id}.txt', as_np=True)[:-1]
        else:
            self.base_mesh = None
            self.raw_gmm = []
...
class GmmStatuses:
    ...
    def __init__(self, opt: options.Options, shape_ids: List[str], render, view_styles: List[ui_utils.ViewStyle]):
...
class MeshGmmStatuses(GmmStatuses):
    ...
    def __init__(self, opt: options.Options, shape_ids: Union[Tuple[str, ...], List[str]], render, view_styles: List[ui_utils.ViewStyle],
                 with_model: bool):
