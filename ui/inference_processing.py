...
def inference_process(opt: options.Options, wake_condition: synchronize.Condition,
                      sleep__condition: synchronize.Condition, status: mp.Value, items: List[int],
...
class InferenceProcess:
    ...
    # def update_mesh_thread(self, fill_ui_mesh: Callable[[V_Mesh], None]):
    #     while value_neq(self.status, UiStatus.Exit):
    #         while value_eq(self.status, UiStatus.Waiting):
    #             with self.sleep_condition:
    #                 print("sleep: set mesh")
    #                 self.sleep_condition.wait()
    #         if value_eq(self.status, UiStatus.SetMesh):
    #             print("wake: set mesh")
    #             mesh = load_mesh(self.shared_meta, self.shared_vs, self.shared_faces)
    #             fill_ui_mesh(mesh)
    #             set_value(self.status, UiStatus.Waiting)
    #     return 0
    ...
    def __init__(self, opt, fill_ui_mesh: Callable[[V_Mesh], None], request_gmm: Callable[[], Tuple[TS, T]],
        ...
        items = list(map(lambda x: int(x.split('_')[1]), items))
        self.model_process = mp.Process(target=inference_process,
                                        args=(opt, self.sleep_condition, self.wake_condition, self.status, items,
                                              self.shared_gmm, self.shared_meta, self.shared_vs, self.shared_faces))
        self.fill_ui_mesh = fill_ui_mesh
        # self.listen_thread = threading.Thread(target=self.update_mesh_thread, args=(fill_ui_mesh,))
