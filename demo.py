...
class RenderMain(StagedCanvas):
    ...
    def __init__(self, opt, viewport:Tuple[float, float, float, float], shape_ids, render_window: vtk.vtkRenderWindow,
...
class MeshScroller(vtk.vtkRenderer):
    ...
    def __init__(self, opt, viewport: Tuple[float, float, float, float], render_window: vtk.vtkRenderWindow, iren,
                 bg_color: ui_utils.RGB_COLOR, shape_nums: Tuple[str, ...]):
        ...
        self.actor_ptr = 0
        view_styles = ui_utils.get_view_styles(len(shape_nums), False)
        self.meshes = ui_controllers.MeshGmmStatuses(opt, shape_nums, self, view_styles, False)
...
def main(with_model: bool = True):
    ...
    # opt = options.Options(tag="airplanes_split").load()
    prefix = ''

    shape_ids = map(lambda x: x[1], files_utils.collect(f"{opt.cp_folder}/occ{prefix}/", '.obj'))
    ...
    # render_window.SetSize(renderWindowGetScreenSize())
    render_window.SetNumberOfLayers(3)
    interactor = vtk.vtkRenderWindowInteractor()
    interactor.SetRenderWindow(render_window)
    renderer_main = RenderMain(opt, (0., .15, 1., 1.), shape_ids, render_window, ui_utils.bg_source_color,
                                          list(ui_utils.bg_target_color) + [200], with_model)
    # background_main = vtk.vtkRenderer()
    # background_main.SetViewport(0., .15, 1., 1.)
    # background_main.SetBackground(*ui_utils.rgb_to_float(ui_utils.bg_source_color))
    # renderer_main.SetLayer(1)
    # background_main.InteractiveOff()
    # render_window.AddRenderer(background_main)
    mesh_scroller = MeshScroller(opt, (0., 0., 1., .15), render_window, interactor, ui_utils.bg_target_color, shape_ids)
